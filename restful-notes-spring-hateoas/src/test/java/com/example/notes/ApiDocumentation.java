	...
	void notesCreateExample() throws Exception {
		...
		String tagLocation = this.mockMvc
			.perform(post("/tags")
				...
				.content(this.jsonMapper.writeValueAsString(tag)))
		...
		this.mockMvc
			.perform(post("/notes")
				.contentType(MediaTypes.HAL_JSON)
				.content(this.jsonMapper.writeValueAsString(note)))
			.andExpect(
				status().isCreated())
			.andDo(this.documentationHandler.document(
				requestFields(
	...
	void noteUpdateExample() throws Exception {
		...
		this.mockMvc
			.perform(patch(noteLocation)
				.contentType(MediaTypes.HAL_JSON)
				.content(this.jsonMapper.writeValueAsString(noteUpdate)))
	...
	void tagUpdateExample() throws Exception {
		...
		this.mockMvc
			.perform(patch(tagLocation)
				.contentType(MediaTypes.HAL_JSON)
				.content(this.jsonMapper.writeValueAsString(tagUpdate)))
			.andExpect(status().isNoContent())
			.andDo(this.documentationHandler.document(
				requestFields(
					fields.withPath("name").description("The name of the tag"))));
